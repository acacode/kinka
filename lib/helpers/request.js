...
function createRequest(method, path, requestOptions, requestData) {
  ...
  if (options.body) {
    console.warn(
      'Property "body" has been removed in the next release kinka. Please use "data" property instead of.'
    )
  ...
  if (options.abortableKey) {
    console.warn(
      'Property "abortableKey" has been removed in the next release kinka. Please use "cancelToken" property instead of.'
    )
  ...
  var request = cancelToken
    ? getAbortableRequest(cancelToken)
    : new XMLHttpRequest()
