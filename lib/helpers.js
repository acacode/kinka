...
/**
 * terminate XMLHttpRequest by cancelKey
 * @param {string} cancelKey
 */
function abortRequest(cancelKey) {
  if (abortableRequests[cancelKey]) {
    abortableRequests[cancelKey].abort()
    removeAbortableKey(cancelKey)
  }
}
 ...
 */
function createAbortableRequest(cancelKey) {
  abortRequest(cancelKey)
  abortableRequests[cancelKey] = new XMLHttpRequest()
  return abortableRequests[cancelKey]
}
 ...
 * @param {string} cancelKey
 */
function removeAbortableKey(cancelKey) {
  abortableRequests[cancelKey] = null
  delete abortableRequests[cancelKey]
}
...
function createRequest(method, path, requestOptions, requestData) {
  ...
  if (options.abortableKey) {
    console.warn(
      'abortableKey property has been deprecated and will be removed in the next release. Please use "cancelKey" instead of.'
    )
    options.cancelKey = options.abortableKey
  }
  var cancelKey = options.cancelKey
  ...
  var request = cancelKey
    ? createAbortableRequest(cancelKey)
    : new XMLHttpRequest()
  return new Promise(function(resolve, reject) {
    ...
    request.onreadystatechange = function() {
      if (request.readyState === 4) {
        ...
        request = null
        if (cancelKey) removeAbortableKey(cancelKey)
      }
    }
