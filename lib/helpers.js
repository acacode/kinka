...
function createRequest(method, path, requestOptions, requestData) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    // better is to use `onreadystatechange`
    request.onloadend = function() {
      if (request.readyState === 4) {
        ...
        var response = createResponse(request, isError, url, customErrorMessage)
 ...
 * @param {string} url
 * @param {string} customErrorMessage - extra message if request has been fails
 * @returns {object}
 */
function createResponse(request, isError, url, customErrorMessage) {
  var status = request.status
  ...
  return {
    ...
    err: isError
      ? status === 0 ? customErrorMessage || 'canceled' : data || {}
      : null,
