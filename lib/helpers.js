 ...
 */
function typeCheck(value, types, message) {
  if (!valueIs(value, types)) {
    console.log(message)
  }
}
...
function createRequest(method, path, requestOptions, requestData) {
  if (process.env.NODE_ENV !== 'production') {
    typeCheck(path, ['String'], 'path in your request should be string')
    typeCheck(
      method,
      ['String'],
      'name of method in your request should be string'
    )
    if (
      typeof requestOptions !== 'undefined' &&
      typeof requestOptions !== 'object'
    ) {
      console.log('requestOptions should be an object')
    }
  }
