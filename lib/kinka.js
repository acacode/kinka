...
/** **********************************************************************/
function typeCheck(value, expectedType, varName, checkOnUndef) {
  if (!checkOnUndef || typeof value !== 'undefined') {
    var actualType = _typeof(value);

    var typeIsInvalid = expectedType === 'array' && !(value instanceof Array) || expectedType === 'object' && (value === null || value instanceof Array || actualType !== expectedType) || actualType !== expectedType;

    if (typeIsInvalid) {
      console.warn('[kinka warning] ' + varName + ' should have type ' + expectedType);
    }
  }
}
function emptyCheck(value, varName) {
  var valueType = _typeof(value);

  var isString = valueType === 'string';
  var isObject = valueType === 'object';

  if (valueType !== 'undefined' && (isObject ? value === null || !Object.keys(value).length : !value)) {
...
if (!ArrayBuffer.isView) {
  ArrayBuffer.isView = function (data) {
    return data !== null && _typeof(data) === 'object' && data.buffer instanceof ArrayBuffer;
  };
}
/*
...
function createRequest(method, path, reqOptions, reqData) {
  ...
  var upperCaseMethod = method.toUpperCase(); // merge instance options with request options
  ...
  typeCheck(onUploadProgress, 'function', 'onUploadProgress', true); // create XMLHttpRequest instance
  ...
  return new Promise(function (resolve, reject) {
    ...
    request.open(upperCaseMethod, url, true); // Listen on download progress if onDownloadProgress callback is presented

    if (onDownloadProgress) request.addEventListener('progress', onDownloadProgress); // Listen on upload progress if onUploadProgress callback is presented

    if (onUploadProgress && request.upload) request.upload.addEventListener('progress', onUploadProgress);

    request.onreadystatechange = function () {
      if (request.readyState === 4) {
        typeCheck(options.successStatus, 'number', 'successStatus', true); // completeRequest
    ...
    request.send(requestBody); // TODO: AT NOW OMIT CATCHES IS NOT WORKING
  });
}
...
function createInstance(config) {
  ...
  for (var x in methods) {
    ...
    if (!instance[method]) instance[method] = createRequest.bind(instance, method);
  }
