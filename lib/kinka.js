...
function typeCheck(value, expectedType, varName, checkOnUndef) {
  if (!checkOnUndef || typeof value !== 'undefined') {
    const actualType = _typeof(value)
...
function createRequest(method, path, reqOptions, reqData) {
  ...
  const upperCaseMethod = method.toUpperCase() // merge instance options with request options
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (onDownloadProgress)
      request.addEventListener('progress', onDownloadProgress) // Listen on upload progress if onUploadProgress callback is presented
